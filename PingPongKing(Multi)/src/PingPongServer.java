...
public class PingPongServer {
    		...
    		try {
    			while (true) {
    				// 읽은 메시지의 종류에 따라 각각 할일이 정해져 있음
           	 		message = (PingPongMessage) reader.readObject();	  // 클라이언트의 전송 메시지 받음
           	 		type = message.getType();
           	 		if (type == PingPongMessage.MsgType.LOGIN) {		  // 클라이언트 로그인 요청
           	 			handleLogin(message.getSender(),writer);	  // 클아이언트 이름과 그에게 메시지를
           	 														  // 보낼 스트림을 등록
           	 		}
           	 		else if (type == PingPongMessage.MsgType.LOGOUT) {	  // 클라이언트 로그아웃 요청
    ...
    private synchronized void handleLogin(String user, ObjectOutputStream writer) {
	   try {
		   ...
		   if (clientOutputStreams.containsKey(user)) {
			   writer.writeObject(
				   new PingPongMessage(PingPongMessage.MsgType.LOGIN_FAILURE, "", "", "사용자 이미 있음"));
			   return;
		   }
	   } catch (Exception ex) {
    ...
    private synchronized void handleMessage(String sender, String receiver, String contents) {
	   ...
	   if (receiver.equals(PingPongMessage.ALL)) {			// "전체"에게 보내는 메시지이면
 ...
 // 클라이언트가 대화 상대방에게 보내는 메시지. 그 상대 혹은 "전체"에게 보내 주어야 함
    ...
    private void broadcastMessage(PingPongMessage message) {
       ...
       Iterator<String> it = s.iterator(); //사용자 리스트 다 쪼갬
       String user; // 사용자 리스트에서 하나씩 담아둘 스트링 객체
       while(it.hasNext()) {
    	   user = it.next();
    	   try {
	           ObjectOutputStream writer = clientOutputStreams.get(user);	// 유저 키에 해당하는 대상 사용자와의 스트림 추출
